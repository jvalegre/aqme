...
class LinearRegressionModel:
    ...
    def process_features_csv(
        ...
        - self.output_name is the target column name (case-insensitive allowed).
        ...
        if output_name is None:
            raise AttributeError("`self.output_name` must be set to the target column name.")
        ...
        target_col = _case_insensitive_find(output_name, df.columns)
        if target_col is None:
            raise KeyError(
                f"Target column {output_name!r} not found (case-insensitive). "
                f"Available: {list(df.columns)}"
            )
