...
try:
    ...
    try:
        ...
        print('Relative Imports Failed : trying absolute imports')
...
def resolve_n_jobs(user_n_jobs: int | None) -> int:
...
def add_modeling_path(modeling_path: str | None):
    """Add the modeling path to sys.path if given (or if a sensible default exists)."""
    default_path = "/gpfs0/gaus/users/edenspec/scripts/M3_modeler"
    path = modeling_path or default_path
    if os.path.isdir(path) and path not in sys.path:
        sys.path.append(path)

def safe_chdir(workdir: str | None, features_csv: str | None):
    """
...
def run_experiment(
    ...
    n_jobs: int = 1,
    min_features_num: int = 4,
    max_features_num: int = 4,
    initial_r2_threshold: float = 0.70,     # regression
    mcfadden_threshold: float = 0.02,       # classification
    top_n: int = 50,
    leave_out: list[str] | None = None,
    ...
    if mode.lower() == "regression":
        if hasattr(model, "search_models"):
            model.search_models(
                top_n=top_n,
                n_jobs=eff_n_jobs,
                initial_r2_threshold=initial_r2_threshold,
        ...
        if hasattr(model, "search_models"):
            model.search_models(
                top_n=top_n,
                n_jobs=eff_n_jobs,
                mcfadden_threshold=mcfadden_threshold,
                bool_parallel=bool_parallel
            )
        else:
...
def main():
    ...
    model_parser.add_argument("-o", "--output_name", default="output", help="Base name for output files.")
    ...
    model_parser.add_argument("--initial-r2", type=float, default=0.70, help="Initial R² threshold (regression).")
    model_parser.add_argument("--mcfadden", type=float, default=0.02, help="McFadden R² threshold (classification).")
    model_parser.add_argument("--leave-out", nargs="*", default=[], help="Space-separated list of samples to leave out.")
    model_parser.add_argument("--modeling-path", default=None, help="Folder containing modeling.py (optional).")
    model_parser.add_argument("--workdir", default=None, help="Working directory (optional).")
    ...
    if args.command == "gui":
        ...
        add_modeling_path(args.modeling_path)
        ...
        elapsed = run_experiment(
            ...
            initial_r2_threshold=args.initial_r2,
            mcfadden_threshold=args.mcfadden,
            top_n=args.top_n,
