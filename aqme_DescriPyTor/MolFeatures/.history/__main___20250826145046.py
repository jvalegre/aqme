...
def run_experiment(
    ...
    """
    import modeling as mf  # import AFTER sys.path is set
    ...
    if mode.lower() == "regression":
        model = mf.LinearRegressionModel(
            csv_filepaths,
    ...
    start = time.time()
    if mode.lower() == "regression":
        if hasattr(model, "fit_and_evaluate_combinations"):
            model.fit_and_evaluate_combinations(
                top_n=top_n,
                n_jobs=eff_n_jobs,
                initial_r2_threshold=initial_r2_threshold
            )
        elif hasattr(model, "search_models"):
            model.search_models(
                top_n=top_n,
                n_jobs=eff_n_jobs,
                initial_r2_threshold=initial_r2_threshold,
            ...
            raise AttributeError("Regression model has neither fit_and_evaluate_combinations nor search_models.")
    else:  # classification
