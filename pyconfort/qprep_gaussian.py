...
import glob
import pandas as pd
from pyconfort.utils import load_from_yaml, Logger
...
class GaussianTemplate():
	...
	def __init__(self, mol=None, destination=os.getcwd(), molecule='', CHARGE=0, MULT=1, chk=False,
				mem='8GB', nprocs=4, ATOMTYPES=[], CARTESIANS=[], bs_gen='', bs='', gen_atoms=None,
	...
	def get_tail(self):
		...
		if self.program.lower() == 'gaussian':
			if len(self.gen_atoms) > 0:
				...
				for _,element_ecp in enumerate(self.ATOMTYPES):
