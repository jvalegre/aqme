...
def get_fragmented_versions(mol,atom_i,atom_j,metal_idx):
    ...
    mol_i = Chem.FragmentOnBonds(mol, [bond_i.GetIdx()],addDummies=True, dummyLabels=[(atom_i, metal_idx)])
    bond_j = mol.GetBondBetweenAtoms(atom_j, metal_idx)
    mol_j = Chem.FragmentOnBonds(mol, [bond_j.GetIdx()],addDummies=True, dummyLabels=[(atom_j, metal_idx)])
    return mol_i,mol_j
...
def passes_Ir_bidentate_x3_rule_angle_requirements(ligand_atoms,mol,metal_idx,offset):
    """
    ...
    if len(ligand_atoms) == 2: # For complexes with 2 Ph_Py ligands + 1 ligand that is not Ph_Py
        ...
        for atom_j in atom_indexes:
            ...
            if atom_i != atom_j and sym_i == 'C':
                ...
                if not is_carbene_like(neighbours):
                    ...
                    mol_i,mol_j = get_fragmented_versions(mol,atom_i,atom_j,metal_idx)
                    ...
                    if not mol_i.GetAtomWithIdx(atom_i).IsInRingSize(5):
                        ...
                        if not mol_i.GetAtomWithIdx(atom_j).IsInRingSize(5):
                            if mol.GetAtomWithIdx(atom_j).IsInRingSize(5):
                                ...
                                break
    # Check Angles
    passing = passing and passes_Ir_bidentate_x3_rule_angle_requirements(ligand_atoms,
                                                                         mol_conf,
                                                                         metal_idx,
                                                                         angle_off)
    return passing
...
def pre_E_filter(sortedcids,cenergy,dup_data,dup_data_idx,log,calc_type,threshold,verbose):
    ...
    if verbose:
        log.write(f"o  {str(eng_dup)} duplicates removed  pre-energy filter (E < {threshold} kcal/mol)")

    if calc_type == 'rdkit': column = 'RDKit-initial_energy_threshold'
    elif calc_type == 'summ': column = 'summ-initial_energy_threshold'
    # if calc_type == 'fullmonte': column = 'FullMonte-initial_energy_threshold'
    elif calc_type == 'ani': column = 'ANI-initial_energy_threshold'
    elif calc_type == 'xtb': column = 'xTB-initial_energy_threshold'
    else: column = ''
...
def RMSD_and_E_filter(outmols,selectedcids_initial,cenergy,args,dup_data,dup_data_idx,log,calc_type):
    ...
    if verbose:
        log.write(f"o  Removing duplicate conformers (RMSD < {rms_threshold} and E difference < {energy_threshold} kcal/mol)")
    ...
    for conf in selectedcids_initial[1:]:
        is_unique = True
        # check energy and rmsd
        for seenconf in selectedcids:

            E_diff = abs(cenergy[conf] - cenergy[seenconf]) # in kcal/mol
            if not (E_diff < energy_threshold): 
                continue

            # At this point we only have conf with E_diff under the threshold. 
            probe = outmols[conf]
            target = outmols[seenconf]
            c1 = c2 = -1
            if calc_type == 'rdkit':
                probe, target = target,probe
                c1,c2 = seenconf, conf
            rms = get_conf_RMS(probe, target, c1, c2, heavyonly, max_matches_RMSD)
    ...
    if verbose:
        log.write(f"o  {eng_rms_dup} duplicates removed (RMSD < {rms_threshold} / E < {energy_threshold} kcal/mol)")
