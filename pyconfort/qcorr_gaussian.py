...
from pyconfort.filter import geom_rules_output
from pyconfort.utils import (move_file, get_info_com, Logger, load_from_yaml, 
...
class qcorr():
	...
	def __init__(self, qm_files=[], round_num=0, w_dir_main=os.getcwd(), 
		...
		self.log = Logger(self.w_dir_main+'/dat_json_files/pyCONFORT',f'QCORR-run_{str(self.round_num)}')
		...
		qcorr_data = self.qcorr_processing()
	...
	def qcorr_processing(self):
		...
		file_name_list, CHARGE_list, MULT_list = [],[],[]
		ATOMTYPES_list, CARTESIANS_list, keywords_line_list = [],[],[]
		...
		for file in self.qm_files:
			...
			TERMINATION, ERRORTYPE, initial_IM_FREQS, NATOMS, CHARGE, MULT, keywords_line, calc_type, nimag_line = self.get_qcorr_params(outlines)
			...
			if ERRORTYPE not in ['before_E_calculation',"atomicbasiserror"] or self.args.nocheck:
				...
				if TERMINATION != "normal" or initial_IM_FREQS != 0 or self.args.s2_threshold > 0.0 or self.args.sp != None or self.args.dup or self.args.isom != None:
					if self.args.nocheck:
						TERMINATION = "normal"
					file_list, E_dup, H_dup, G_dup, ERRORTYPE, keywords_line, ATOMTYPES, CARTESIANS = self.get_input_geom(file, outlines, keywords_line, NATOMS, MULT, TERMINATION, ERRORTYPE, initial_IM_FREQS, nimag_line, calc_type, file_list, E_dup, H_dup, G_dup)
				...
				if self.args.isom != None:
					...
					try:
						...
						isomerized = check_isomerization(coords_com, CARTESIANS, atoms_com, ATOMTYPES, self.args.vdwfrac, self.args.covfrac, init_csv, file)
		...
		qcorr_data_dict = {'file_name': file_name_list, 'CHARGE': CHARGE_list, 'MULT': MULT_list,
					'ATOMTYPES': ATOMTYPES_list, 'CARTESIANS': CARTESIANS_list, 'keywords_line': keywords_line_list,
					'TERMINATION': TERMINATION_list, 'ERRORTYPE': ERRORTYPE_list}
		
		df_qcorr = pd.DataFrame(qcorr_data_dict)
		df_qcorr_success = df_qcorr[df_qcorr['ERRORTYPE'] == None]
		df_qcorr_error = df_qcorr[df_qcorr['ERRORTYPE'] != None]

		# creates json objects and saves the files, updating any previous json files
		check for json files
		load them into pd, different names since df_qcorr_success/df_qcorr_error wont be modified! 
		simultaneous
			check if the entries in the new success existed in the previous file
				if they dont exist, add them as new sucess
			check if the normal terminated files existed as errors before
				if they existed, delete them
		create json files with run+1
		

		with open(f'{self.w_dir_main}/dat_json_files/qcorr_success_{self.round_num}.json', 'w') as f:
			json.dump(df_qcorr_success, f) 
		with open(f'{self.w_dir_main}/dat_json_files/qcorr_error_{self.round_num}.json', 'w') as f:
			json.dump(df_qcorr_error, f) 
	...
	def get_qcorr_params(self,outlines):
		...
		if TERMINATION != "normal" or initial_IM_FREQS > 0  or self.args.sp != None or self.args.isom != None or self.args.nocheck:
			NATOMS,CHARGE,MULT,keywords_line,initial_IM_FREQS,calc_type = self.get_init_info(outlines,initial_IM_FREQS)
	...
	def get_init_info(self,outlines,initial_IM_FREQS):
		"""
	...
	def get_input_geom(self, file, outlines, keywords_line, NATOMS, MULT, TERMINATION, ERRORTYPE, initial_IM_FREQS, nimag_line, calc_type, file_list, E_dup, H_dup, G_dup):
		"""
		...
		if ERRORTYPE == 'before_E_calculation':
			...
			if TERMINATION == "normal":
				stand_or,ERRORTYPE,FREQS,NORMALMODE,file_list,E_dup,H_dup,G_dup = self.analysis_normal(file,outlines,stand_or,ERRORTYPE,nimag_line,NATOMS,file_list,E_dup,H_dup,G_dup,initial_IM_FREQS,MULT,calc_type)
			...
			if ERRORTYPE == 'extra_imag_freq':
				CARTESIANS = self.fix_imag_freqs(NATOMS, CARTESIANS, FREQS, NORMALMODE)
			
			return file_list, E_dup, H_dup, G_dup, ERRORTYPE, keywords_line, ATOMTYPES, CARTESIANS


	def analysis_normal(self,file,outlines,stand_or,ERRORTYPE,nimag_line,NATOMS,file_list,E_dup,H_dup,G_dup,initial_IM_FREQS,MULT,calc_type):
		"""
	...
	def fix_imag_freqs(self, NATOMS, CARTESIANS, FREQS, NORMALMODE):
		"""
