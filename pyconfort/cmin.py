...
from pyconfort.utils import (set_metal_atomic_number,Logger,rules_get_charge,
 							substituted_mol,creation_of_dup_csv_cmin,load_from_yaml)
...
class cmin():
	...
	def __init__(self, mols=None, name=None, w_dir_initial=os.getcwd(), yaml_file=None, charge_defualt=0, program='xtb',**kwargs):
		self.mols = mols
		self.name = name
	...
	def compute_cmin(self):
		...
		for i,mol in enumerate(self.mols):
			...
			if mol is not None:
				...
				if self.args.metal_complex:
					...
					self.args.mol,self.args.metal_idx,self.args.complex_coord,self.args.metal_sym = substituted_mol(mol,self.args)

				self.mol, energy, ani_incompatible = self.optimize(mol, self.args, self.program,self.log,dup_data,dup_data_idx)
		...
		for cid in sorted_all_cids:
			outmols[cid].SetProp('_Name', outmols[cid].GetProp('_Name') +' '+self.program)
		...
		cmin_file = cmin_folder.joinpath(f'{name_mol}_{self.program}_all_confs{self.args.output}')
		...
		sortedcids = ewin_filter(sorted_all_cids,cenergy,self.args,dup_data,dup_data_idx,self.log,self.program,self.args.ewin_csearch)
		# pre-filter based on energy only
		selectedcids_initial = pre_E_filter(sortedcids,cenergy,dup_data,dup_data_idx,self.log,self.program,self.args.initial_energy_threshold,self.args.verbose)
		# filter based on energy and RMSD
		selectedcids = RMSD_and_E_filter(outmols,selectedcids_initial,cenergy,self.args,dup_data,dup_data_idx,self.log,self.program)
		...
		self.write_confs(outmols, cenergy, selectedcids, name_mol, self.args, self.program,self.log,cmin_folder)
	...
	def optimize(self,mol, args, program,log,dup_data,dup_data_idx):
		...
		if program == 'ani':
			...
			energy, coordinates = self.xtb_calc(elements,coordinates,args,log,ase_metal,ase_metal_idx)
		else:
	...
	def write_confs(self,conformers, energies,selectedcids, name, args, program,log,cmin_folder):
		if len(conformers) > 0:
