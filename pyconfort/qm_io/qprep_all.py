...
from pyconfort.utils import periodic_table,load_from_yaml,Logger
...
class GaussianTemplate():
	...
	def __init__(self, mol=None, destination=os.getcwd(), molecule='', CHARGE=0,
				MULT=1, ATOMTYPES=[], CARTESIANS=[], bs_gen='',
	...
	def get_tail(self):
		...
		for _,element_ecp in enumerate(gen_atoms):
			if element_ecp in self.ATOMTYPES and element_ecp not in ecp_used:
				ecp_used.append(element_ecp)
			elif element_ecp not in self.ATOMTYPES and element_ecp not in ecp_used:
	...
	def write(self):
			...
			for atom in range(0,self.NATOMS):
				fileout.write('{0:>2} {1:12.8f} {2:12.8f} {3:12.8f}'.format(self.ATOMTYPES[atom], self.CARTESIANS[atom][0], self.CARTESIANS[atom][1], self.CARTESIANS[atom][2]))
				fileout.write("\n")
			fileout.write("\n")
	...
	if charge_com != 'Invalid':
		...
		for file in com_files:
			...
			if args.QPREP == 'gaussian':
				...
				if genecp != 'None':
					write_genecp(ATOMTYPES,type_gen,fileout,genecp,ecp_list,ecp_gen_atoms,ecp_gen_atoms,bs,lot,bs_gcp,args,w_dir_initial,path_write_input_files)
			...
			if args.QPREP == 'orca':
				...
				ecp_list,ecp_gen_atoms,ecp_gen_atoms,genecp,orca_aux_section = check_for_gen_or_genecp(ATOMTYPES,args,'analysis','orca')
