...
class TurbomoleOutput(object):
    ...
    def guess_calculation_type(self):
        ...
        -------
        str
            5 possible outcomes: 'rirpa','sp','opt','freq', 'opt+freq'
        """
        calc_type = []
        file = self.folder / 'rirpa.out'
        if file.exists():
            return 'rirpa'
    ...
    def _guess_calculation_state_sp(self):
        ...
        """
        assert self.calc_type in ['sp', 'rirpa']
        if self.calc_type == 'sp':
            file = self.folder / 'ridft.out'
        else:  # self.calc_type == 'rirpa'
            file = self.folder / 'rirpa.out'
        ...
        for line in lines:
            has_failed = 'ridft did not converge!' in line
            has_failed = has_failed or 'rirpa did not converge!'
            if has_failed: 
                return 'failed'
        
        if 'ended normally' in lines[-1]:
            return 'converged'
    ...
    def parse_energy(self):
        ...
        if self.cosmo: 
            ...
            pattern = r'\|.*?total\s*?energy\s*?\=\s*?(\-+[0-9]*\.[0-9]*)\s*?\|'
...
class TurbomoleInput(object):
    ...
    # Generation methods
    def write_input_command(self,script=None):
        """
