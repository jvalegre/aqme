...
[
    ...
    ('charge_mult', 'json_files', 'com_files', False), # test json inputs with defined charge and mult
    ...
    ('genecp_and_final', 'json_files', 'gen_final_files', False), # test genecp and final line
...
def test_QPREP_analysis(test_type, init_folder, target_folder, restore_folder):
    ...
    if test_type == 'com_gen':
        ...
        for file in files_assert:
            ...
            if file.split('.')[0] == 'H_freq':
                ...
                if file.split('.')[0] == 'CH4':
                    ...
                    assert len(glob.glob(f'{destination}/quinine_rdkit_conf_*.com')) == 10
        ...
        if init_folder == 'json_files':
            ...
            files_assert = ['quinine_rdkit_charged_conf_1.com', 'quinine_rdkit_charged_conf_10.com']
        elif init_folder == 'xyz_files':
        ...
        cmd_aqme = ['python', '-m', 'aqme', '--qprep', '--w_dir_main', w_dir_main, '--destination', destination,
        '--files', 'CH4.log', '--program', 'orca', '--qm_input', ORCA_SP]
        subprocess.run(cmd_aqme)
        ...
        assert glob.glob(f'{destination}/*.inp')[0].replace('\\','/').split('/')[-1] == 'CH4.inp'
        ...
        cmd_aqme = ['python', '-m', 'aqme', '--qprep', '--w_dir_main', w_dir_main, '--destination', destination,
        '--files', 'CH4.json', '--program', 'gaussian', '--qm_input', 'wb97xd/lanl2dz scrf=(smd,solvent=acetonitrile)',
        '--charge', '2', '--mult', '3', '--suffix', 'params', '--chk', '--mem', '100GB', '--nprocs', '32']
        subprocess.run(cmd_aqme)  
        ...
        cmd_aqme = ['python', '-m', 'aqme', '--qprep', '--w_dir_main', w_dir_main, '--destination', destination,
        '--files', 'CH4.json', '--program', 'gaussian', '--qm_input', 'pop=(nbo6read,savenbos) wb97xd/def2svp',
        '--qm_end', '$nbo bndidx $end', '--suffix', 'final']
        subprocess.run(cmd_aqme)  
        ...
        cmd_aqme = ['python', '-m', 'aqme', '--qprep', '--w_dir_main', w_dir_main, '--destination', destination,
        '--files', 'CH4.json', '--program', 'gaussian', '--qm_input', 'pop=(nbo6read,savenbos) wb97xd/genecp',
        '--qm_end', '$nbo bndidx $end', '--suffix', 'genecp_and_final', '--gen_atoms', "['C']", '--bs_gen', 'LANL2TZ', '--bs_nogen', 'LANL2DZ']
        subprocess.run(cmd_aqme)  
        ...
        cmd_aqme = ['python', '-m', 'aqme', '--qprep', '--w_dir_main', w_dir_main, '--destination', destination,
        '--files', 'CH4.json', '--program', 'gaussian', '--qm_input', 'wb97xd/gen',
        '--suffix', 'gen', '--gen_atoms', "['C']", '--bs_gen', 'LANL2TZ', '--bs_nogen', 'LANL2DZ']
        subprocess.run(cmd_aqme)  
        ...
        cmd_aqme = ['python', '-m', 'aqme', '--w_dir_main', w_dir_main, '--destination', destination, '--varfile', f'{w_dir_main}/pytest_varfile.yaml']
        subprocess.run(cmd_aqme)  
