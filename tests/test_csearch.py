...
def test_csearch_input_parameters(program, input, output_nummols):
	...
	if input in ['pentane.smi','pentane.csv']:
		file1= str("CSEARCH/" + program + "/" +"butane_"+input.split('.')[1]+"_" + program + ".sdf")
		file2= str("CSEARCH/" + program + "/" +"pentane_" +input.split('.')[1]+"_" + program + ".sdf")
		...
		file= str("CSEARCH/" + program + "/" +input.split('.')[0]+"_0_" + program + ".sdf")
		mols = rdkit.Chem.SDMolSupplier(file,removeHs=False)
		assert len(mols) == output_nummols
	else:
		file= str("CSEARCH/" + program + "/" +input.split('.')[0]+"_" + program + ".sdf")
		mols = rdkit.Chem.SDMolSupplier(file,removeHs=False)
		assert len(mols) == output_nummols
	os.chdir(w_dir_main)
...
def test_csearch_others_parameters(program, smi, name, max_matches_rmsd , max_mol_wt , ff, degree, verbose, output, max_torsions, prefix, output_nummols ):
	os.chdir(csearch_others_dir)
	# runs the program with the different tests
	csearch(w_dir_main=csearch_others_dir, program=program, smi=smi, name=name,max_matches_rmsd=max_matches_rmsd,
	max_mol_wt=max_mol_wt,ff=ff,degree=degree,verbose=verbose,output=output,max_torsions=max_torsions,prefix=prefix)

	#tests here
	file = str("CSEARCH/" + program + "/" + prefix +"_" +name + "_" + program + ".sdf")
	mols = rdkit.Chem.SDMolSupplier(file,removeHs=False)
	assert len(mols) == output_nummols
	os.chdir(w_dir_main)
...
@pytest.mark.parametrize(
	"program, smi, name, cbonds, cregen, cregen_ethr, cregen_rthr , cregen_bthr, cregen_ewin, output_nummols",
	[
		# tests for conformer generation with RDKit
		("crest", "CCCCC", "pentane", 0.8, True, 1, 0.5, 0.7, 4, 4),
	],
)
def test_csearch_crest_parameters(program, smi, name,cbonds,cregen,cregen_ethr,cregen_rthr,cregen_bthr,cregen_ewin, output_nummols):
	os.chdir(csearch_crest_dir)
	# runs the program with the different tests
	csearch(w_dir_main=csearch_crest_dir, program=program, smi=smi, name=name,cbonds=cbonds,cregen=cregen,cregen_ethr=cregen_ethr,cregen_rthr=cregen_rthr,cregen_bthr=cregen_bthr,cregen_ewin=cregen_ewin)
...
def test_csearch_fullmonte_parameters(program, smi, name, charge, mult, ewin_fullmonte, ewin_sample_fullmonte, nsteps_fullmonte, nrot_fullmonte, ang_fullmonte,output_nummols):
	os.chdir(csearch_fullmonte_dir)
	# runs the program with the different tests
	csearch(w_dir_main = csearch_fullmonte_dir, program=program, smi=smi, name=name,charge=charge,mult=mult,ewin_fullmonte=ewin_fullmonte,ewin_sample_fullmonte=ewin_sample_fullmonte,
	nsteps_fullmonte=nsteps_fullmonte,nrot_fullmonte=nrot_fullmonte,ang_fullmonte=ang_fullmonte,output_nummols=output_nummols)
...
@pytest.mark.parametrize(
	...
	[
		# tests for conformer generation with RDKit
		("rdkit", "CCCCC", "pentane_rdkit", 0, 1, 'auto', 1000 , False, 10, 0.0001, 2, 0.2, 4),
		("summ", "CCCCC", "pentane_summ", 0, 1, 'auto', 100 , True, 40, 0.0001, 0.2, 0.1, 5),
		("rdkit", "CC[CH]CC", "radical_rdkit", 0, 2, 'auto', 10 , True, 10, 0.000001, 0.6, 0.3, 41),
		("summ", "CC[CH]CC", "radical_summ", 0, 2, 'auto', 500 , True, 2, 0.0001, 0.2, 0.2, 3),
		("rdkit", "C[NH2+]CC", "charged_rdkit", 1, 0, 'auto', 10 , True, 10, 0.0001, 4, 0.6, 5),
		("summ", "C[NH2+]CC", "charged_summ", 1, 0, 'auto', 1000 , False, 10, 0.0001, 0.2, 0.2, 2),

	],
)
def test_csearch_rdkit_summ_parameters(program, smi, name, charge, mult, sample, opt_steps_rdkit, heavyonly,
 ewin_csearch, initial_energy_threshold, energy_threshold, rms_threshold, output_nummols):
	os.chdir(csearch_rdkit_summ_dir)
	# runs the program with the different tests
	csearch(w_dir_main =csearch_rdkit_summ_dir, program=program, smi=smi, name=name,charge=charge,mult=mult,sample=sample,opt_steps_rdkit=opt_steps_rdkit, heavyonly= heavyonly, ewin_csearch =ewin_csearch,
	initial_energy_threshold=initial_energy_threshold, energy_threshold=energy_threshold, rms_threshold=rms_threshold, output_nummols=output_nummols)
...
@pytest.mark.parametrize(
	...
	[
		...
		("rdkit", "CCCCC", "pentane", False, False, None, None, None, [], [], [], False,4),
		("summ", "CCCCC", "pentane", False, False, None, None, None, [], [], [], False,4),
		(
		    "fullmonte",
		    "CCCCC",
		    "pentane",
		...
		("crest", "CCCCC", "pentane", False, False, None, None, None, [], [], [], True, 8),
		# (
		#     "rdkit",
...
def test_csearch_methods(
	...
	if not complex and not metal_complex:
		csearch(w_dir_main =csearch_methods_dir,program=program, smi=smi, name=name,cregen=cregen)
