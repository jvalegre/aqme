...
from aqme.utils import periodic_table, move_file
...
def getSHIELDING(outlines,args,log):
  ...
  return NMR, cartesians,n_atoms,atom_types
...
def calculate_nics_parameters(qm_files,args,log,w_dir_initial,name_mol,lot,bs):
    ...
    for counter,log in enumerate(qm_files):
        ...
        NMR, cartesians,n_atoms,atom_types = getSHIELDING(outlines,args,log)
        vector_norm,centroid = update_coord(n_atoms,atom_types,cartesians,args,log,name_mol,w_dir_initial,'read')
...
def calculate_boltz_for_nics(val,args,log,name,w_dir_fin,w_dir_initial,lot,bs):
    ...
    if str(bs).find('/') > -1:
        destination = Path(w_dir_initial+'/QPRED/nics/boltz/'+str(lot)+'-'+str(bs).split('/')[0])
    else:
        destination = Path(w_dir_initial+'/QPRED/nics/boltz/'+str(lot)+'-'+str(bs))
