...
from .templates import qprep,OrcaTemplate,TurbomoleTemplate
...
def check_for_gen_or_genecp(atom_types,args,type_of_check,program_gen):
	# Options for genecp
	...
	for _,atomtype in enumerate(atom_types):
		if program_gen == 'orca':
...
def qcorr(qm_files,com_files, w_dir, w_dir_main,lot, bs, bs_gcp, args, w_dir_fin, w_dir_initial, log, ana_data, round_num):
	...
	for file in qm_files: # Ideally the in_file and out_file should be iterated in parallel
		...
		if len(args.geom_rules) >= 1:
			if termination == "normal" and ifreqs == 0:
		...
		finished,unfinished,atom_error,scf_error,imag_freq,other_error,geom_rules_qcorr,check_geom_qcorr = organize_outputs(w_dir,w_dir_main,round_num,file,ifreqs,termination,errortype,w_dir_fin,finished,unfinished,atom_error,scf_error,imag_freq,other_error,geom_rules_qcorr,passing_rules,passing_geom,check_geom_qcorr)
		...
		if ifreqs > 0 or termination != "normal" and not os.path.exists(w_dir_main+'/failed/run_'+str(round_num)+'/error/basis_set_error/'+file):
			create_folder_and_com(w_dir,w_dir_main,round_num,log,n_atoms,atom_types,cartesians,args,termination,ifreqs,w_dir_fin,file,lot,bs,bs_gcp,ecp_list,ecp_gen_atoms,ecp_gen_atoms,genecp,errortype,input_route,w_dir_initial,name,charge, mult, orca_aux_section)

		# adding in the NMR componenet only to the finished files after reading from normally finished log files
		if termination == "normal" and ifreqs == 0 and passing_rules and passing_geom:
			if args.sp == 'gaussian' or args.sp == 'orca' or args.sp == 'turbomole' or args.nics:

				#get coordinates
				atom_types, cartesians,stand_or = get_coords(outlines, stand_or, n_atoms, periodic_table, atom_types, cartesians)
				...
				# Options for genecp
				if args.sp == 'gaussian':
					ecp_list,ecp_gen_atoms,ecp_gen_atoms,genecp,orca_aux_section = check_for_gen_or_genecp(atom_types,args,'sp','gaussian')

				elif args.sp == 'orca':
					ecp_list,ecp_gen_atoms,ecp_gen_atoms,genecp,orca_aux_section = check_for_gen_or_genecp(atom_types,args,'sp','orca')
				...
				for lot_sp,bs_sp,bs_gcp_sp in zip(args.level_of_theory_sp,args.basis_set_sp,basis_set_for_genecp):
					...
					if args.sp == 'gaussian' or args.sp == 'orca' or args.sp == 'turbomole':
						...
						new_com_file('sp',w_dir_initial,log,single_point_input_files+'/'+dir_name,file,args,keywords_opt,name,charge,mult,n_atoms,atom_types,cartesians,genecp,ecp_list,ecp_gen_atoms,ecp_gen_atoms,termination,ifreqs,bs_sp,lot_sp,bs_gcp_sp,orca_aux_section)

					if args.nics:
						if not os.path.isdir(nics_input_files+'/'+dir_name):
							os.makedirs(nics_input_files+'/'+dir_name)
						os.chdir(nics_input_files+'/'+dir_name)
						new_com_file('nics',w_dir_initial,log,nics_input_files+'/'+dir_name,file,args,keywords_opt,name,charge,mult,n_atoms,atom_types,cartesians,genecp,ecp_list,ecp_gen_atoms,ecp_gen_atoms,termination,ifreqs,bs_sp,lot_sp,bs_gcp_sp,orca_aux_section)
...
def main(duplicates,w_dir_initial,args,log):
	...
	if args.sp == 'gaussian':
		sp_qm_folder = Path(f'{w_dir_initial}/QCALC/G16-SP_input')
		template_sp = qprep.from_args(args)
	elif args.sp == 'orca':
		sp_qm_folder = Path(f'{w_dir_initial}/QCALC/ORCA')
		template_sp = OrcaTemplate.from_args(args)
	elif args.sp == 'turbomole':
		sp_qm_folder = Path(f'{w_dir_initial}/QCALC/TURBOMOLE')
		template_sp = TurbomoleTemplate.from_args(args)

	# when you run analysis in a folder full of output files
	QCALC_folder = Path(f'{w_dir_initial}/QCALC')
	if not QCALC_folder.exists():
		...
		for lot,bs,bs_gcp in zip(args.level_of_theory, args.basis_set,args.genecp_bs):
			...
			w_dir,n_run = check_run(w_dir_main)
			os.chdir(w_dir)
			log = Logger(f'{w_dir_main}/dat_files/aqme-QCORR-run_{n_run}', args.output_name)
	...
	# when you specify multiple levels of theory
		...
		folders = [item for item in (QCALC_folder/'G16').iterdir() if item.isdir()]
		dat_folders = []
		for folder in folders: 
			dat_folder = folder/'dat_files'
			dat_folder.mkdir(exist_ok=True)
			last_run, n_run = check_run(str(folder))
			dat_file = dat_folder/f'aqme-QCORR-run_{n_run:02d}'
