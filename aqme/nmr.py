        ...
        for i in range(start, stop):
            ...
            if len(split_line) == 3:
                if len(split_line[1].split("-")) > 1:
                    ...
                    split_line_atom[len(split_line_atom) - 1] = (
                        split_line_atom[len(split_line_atom) - 1].strip().split("\\")[0]
                    )
                    for idx, shield in zip(conf_idx, final_shieldings):
                        if idx in split_line_atom:
                            split_line_sheild_min = np.array(
                                split_line_sheild, dtype=float
                            ) - float(shield)
                            idx_min = np.argmin(abs(split_line_sheild_min))
                            if split_line_atom[idx_min] in atom_num:
                                if split_line_atom[idx_min] != idx:
                                    atom_num.append(idx)
                                    atom_sheilding.append(
                                        split_line_sheild[split_line_atom.index(idx)]
                                    )
                                else:
                                    if idx_min == 1:
...
def calculate_nmr(
    nmr_qm_files, args, log, name, w_dir_fin, w_dir_initial, lot_sp, bs_sp, lot, bs
):

    # define the lot etc.,
    [opt_method, opt_basis, opt_solv] = [
        lot,
        bs,
        [args.solvent_model, args.solvent_name],
    ]
    [nmr_method, nmr_basis, nmr_solv] = [
        lot_sp,
        bs_sp,
        [args.solvent_model_sp, args.solvent_name],
    ]
    ...
    if args.solvent_model == "gas_phase":
        ...
        log.write(
            "\no  Optimization: {0}/{1} ({2}, solvent = {3})".format(
                opt_method, opt_basis, opt_solv[0], opt_solv[1]
            )
        )
    ...
    if args.solvent_model_sp == "gas_phase":
        ...
        log.write(
            "\no  NMR calculation: {0}/{1} ({2}, solvent = {3})".format(
                nmr_method, nmr_basis, nmr_solv[0], nmr_solv[1]
            )
        )
    ...
    if args.nmr_online:
        ...
        for nuc in args.nmr_nucleus:
            ...
            try:
                ...
                log.write(
                    "x   No scaling factors found for this level of theory! Input the values as arguments for AQME!"
                )
                exit()
        ...
        for i in range(start, stop):
            try:
                if outlines[i].split()[1] in args.nmr_nucleus:
                    ...
                    if args.nmr_online:
                        scaled_nmr = (intercept_nuc - float(outlines[i].split()[4])) / (
                            -slope_nuc
                        )
    ...
    if args.nmr_exp != "None":
        ...
        atom_num_exp, atom_sheilding_exp = get_exp_data(
            args, name, w_dir_initial, final_shieldings, conf_idx, conf_sym
        )

        # make final array with atom_num, exp, dft
        df = pd.DataFrame(
            {"Atom": atom_num_exp, "Shielding-Exp": atom_sheilding_exp},
        ...
        for i, atom_num in enumerate(atom_num_exp):
            for j, atom_cal in enumerate(conf_idx):
                if len(atom_num.split("-")) > 1:
                    if (
                        atom_cal == atom_num.split("-")[0]
                        or atom_cal == atom_num.split("-")[1]
                        or atom_cal == atom_num.split("-")[2]
                    ):
        ...
        for atom_nuc in args.nmr_nucleus:
            ...
            mae_nuc, sd_nuc = nmr_stats(
                df_nuc["Shielding-Exp"], df_nuc["Shielding-Calc"]
            )
            log.write(
                "\no  The MAE and SD for molecule {0} is {1} and {2} respectively for {3}".format(
                    name, mae_nuc, sd_nuc, atom_nuc
                )
            )
        ...
        df = pd.DataFrame(
            {
                "Atom": conf_idx,
                "Atom-Symbol": conf_sym,
                "Shielding-Calc": final_shieldings,
            },
            columns=["Atom", "Atom-Symbol", "Shielding-Calc"],
        )
    ...
    export_param_excel = df.to_csv(
        name + "_" + lot_sp + "-" + bs_sp + "_predicted_shifts.csv", index=None
    )
