...
from aqme.filter import (filters, ewin_filter,
...
from aqme.fullmonte import (generating_conformations_fullmonte,
								 minimize_rdkit_energy, realign_mol)
from aqme.utils import Logger, set_metal_atomic_number, com_2_xyz_2_sdf, getDihedralMatches, load_from_yaml,creation_of_dup_csv,check_charge_smi,check_for_pieces
...
def process_csearch(mol_, name_, dir_, varfile_, charge_default_ ):
	obj = csearch(mol_,name_,dir_, varfile_,charge_default_)
...
class csearch():
	...
	def __init__(self, mol=None, name=None, w_dir_initial=os.getcwd(), yaml_file=None, charge_defualt=0, **kwargs):
		self.mol = mol
		assert self.mol,'x  Wrong SMILES string ({smi}) found (not compatible with RDKit or ANI/xTB if selected)! This compound will be omitted'
		self.name = name
	...
	def compute_confs(self):
		...
		if self.args.metal_complex:
			...
			self.args.mol, self.args.metal_idx, self.args.complex_coord, self.args.metal_sym = substituted_mol(self.mol,self.args)
			...
			if self.args.complex_type in accepted_complex_types:
				...
				if count_metals == 1:
					...
					total_data = creation_of_dup_csv(self.args.CSEARCH,self.args.CMIN)

					for mol_obj, name_in, coord_map, alg_map, template in zip(*items):
						data = self.conformer_generation(mol_obj, name_in, self.args, log,
				...
				total_data = self.conformer_generation(self.mol,self.name,self.args,self.log)
		else:
			total_data = self.conformer_generation(self.mol,self.name,self.args,self.log)
		return total_data


	def conformer_generation(self,mol,name,args,log,coord_Map=None,alg_Map=None,mol_template=None):
		...
		if valid_structure:
			...
			try:
				...
				status,update_to_rdkit = self.summ_search(mol, name,args,log,dup_data,dup_data_idx,coord_Map,alg_Map,mol_template)
	...
	def summ_search(self, mol, name,args,log,dup_data,dup_data_idx, coord_Map = None, alg_Map=None, mol_template=None):
		...
		status,rotmatches,update_to_rdkit = self.rdkit_to_sdf(mol, name,args,log,dup_data,dup_data_idx, coord_Map, alg_Map, mol_template)
		...
		if status != -1 or status != 0:
			...
			if args.CSEARCH=='summ' and len(rotmatches) != 0:
				status = self.dihedral_filter_and_sdf(name,args,log,dup_data,dup_data_idx,coord_Map, alg_Map, mol_template)
	...
	def dihedral_filter_and_sdf(self,name,args,log,dup_data,dup_data_idx,coord_Map, alg_Map, mol_template):
		"""
	...
	def clean_args(self,args,ori_ff,mol,ori_charge):                                # RAUL: I hope this function does not survive the clean-ups
		"""
	...
	def genConformer_r(self,mol, conf, i, matches, degree, sdwriter,args,name,log,update_to_rdkit,coord_Map,alg_Map, mol_template):
		"""
		...
		if i >= len(matches): # base case, torsions should be set in conf
			...
			while deg < 360.0:
				...
				total += self.genConformer_r(mol, conf, i+1, matches, degree, sdwriter,args,name,log,update_to_rdkit,coord_Map,alg_Map, mol_template)
				deg += degree
			return total

	def embed_conf(self,mol,initial_confs,args,log,coord_Map,alg_Map, mol_template):
		"""
	...
	def min_after_embed(self,mol,cids,name,initial_confs,rotmatches,dup_data,dup_data_idx,sdwriter,args,log,update_to_rdkit,coord_Map,alg_Map, mol_template):
		"""
		...
		outmols,cenergy = self.min_and_E_calc(mol,cids,args,log,coord_Map,alg_Map,mol_template)
		...
		if args.CSEARCH=='summ' or args.CSEARCH=='rdkit':
			...
			for conf in selectedcids_rdkit:
				...
				total += self.genConformer_r(outmols[conf], conf, 0, rotmatches, args.degree, sdwriter ,args,outmols[conf].GetProp('_Name'),log,update_to_rdkit,coord_Map,alg_Map, mol_template)
				# bar.next()
			# bar.finish()
			if args.verbose and len(rotmatches) != 0:
	...
	def rdkit_to_sdf(self,mol, name,args,log,dup_data,dup_data_idx, coord_Map, alg_Map, mol_template):
		...
		if args.CSEARCH !='crest':
			...
			if args.CSEARCH=='rdkit':
				rotmatches =[]
			cids = self.embed_conf(mol,initial_confs,args,log,coord_Map,alg_Map, mol_template)
			...
			status = self.min_after_embed(mol,cids,name,initial_confs,rotmatches,dup_data,dup_data_idx,sdwriter,args,log,update_to_rdkit,coord_Map,alg_Map, mol_template)
		else:
...
def prepare_smiles_files(args,w_dir_initial):
	...
	for i, line in enumerate(lines):
		mol,name,args= prepare_smiles_from_line(line,i,args)
		obj = mol,name,w_dir_initial,args.varfile,args.charge_default
...
def generate_mol_from_csv(args,w_dir_initial,csv_smiles,index):
	...
	obj = csearch(mol,name,w_dir_initial,args.varfile,args.charge_default)
	return obj
...
def prepare_mol_files(args,w_dir_initial): # The extra variables are for API Consistency.
	suppl, IDs, charges = mol_from_sdf_or_mol_or_mol2(args.input)
	job_inputs = []
