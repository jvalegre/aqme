...
class qcorr():
	...
	def __init__(self, qm_files=[], round_num=0, w_dir_main=os.getcwd(), 
				mem='', nprocs=0, chk=False, yaml_file=None, qm_input='',
				fullcheck=False, **kwargs):
	...
	def qcorr_processing(self):
		...
		for file in self.qm_files:
			...
			keywords_line,calc_type,mem,nprocs,program,author = self.get_init_info(outlines)
			...
			if termination == 'normal':
				symmno,point_group,roconst,rotemp = self.symm_rot_data(outlines,program)
			...
			# for calcs with finished OPT but no freqs
				...
				qcorr_calcs = qprep(molecule=file_name, charge=charge, mult=mult,
									program=program, atom_types=atom_types,
									cartesians=cartesians, qm_input=keywords_line,
									mem=mem, nprocs=nprocs, chk=self.chk)
	...
	*funcion check dentro de qcorr aparte*
	...
	def get_init_info(self,outlines):
		...
		return keywords_line,calc_type,mem,nprocs,program,symmno,point_group,roconst,rotemp,author
