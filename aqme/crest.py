...
def run_xtb(xyzin, xyzoutxtb, constraints_dist, constraints_angle, constraints_dihedral, charge,mult):
	...
	if os.path.exists(xyzoutxtb):
		...
		command = [os.path.abspath(os.path.dirname(__file__))+'/run_xtb.sh', xyzin, '--xyzout', str(xyzoutxtb), '--charge', str(charge), '--uhf',str(mult - 1),]
...
def crest_opt(mol, name, dup_data, dup_data_idx, sdwriter, args, charge, mult):
	...
	if args.complex:
		...
		run_xtb(xyzin, xyzoutxtb1, all_fix, constraints_angle, constraints_dihedral, charge, mult)
		run_xtb(xyzoutxtb1, xyzoutxtb2, constraints_dist, constraints_angle, constraints_dihedral, charge,mult)
	else:
		xyzoutxtb2 = xyzin
	...
	if cregen:
		xyzcregenensemble = str(dat_dir) + "/" + name + "_conformers_cregen.xyz"
		xyzcregenbest = str(dat_dir) + "/" + name + "_best_cregen.xyz"
