...
class cmin:
    ...
    def compute_cmin(self):
        ...
        if self.args.charge is None:
            self.args.charge = rules_get_charge(self.mols[0], self.args, "cmin")
        if self.args.mult is None:
            NumRadicalElectrons = []
            for Atom in self.mols[0].GetAtoms():
                NumRadicalElectrons.append(Atom.GetNumRadicalElectrons())

        self.args.mult = NumRadicalElectrons

        TotalElectronicSpin = np.sum(NumRadicalElectrons) / 2
        final_mult = int((2 * TotalElectronicSpin) + 1)

        dup_data.at[dup_data_idx, "Overall charge"] = np.sum(self.args.charge)
    ...
    def xtb_calc(self, elements, coordinates, args, log):
        """
        ...
        for i, atom in enumerate(ase_molecule):
            ...
            atom.charge = args.charge[i]
            atom.magmom = args.mult[i]
    ...
    def optimize(self, mol, args, program, log, dup_data, dup_data_idx):
        ...
        if program == "ani":
            ...
            energy, coordinates = self.xtb_calc(elements, coordinates, args, log)
        else:
            log.write(
