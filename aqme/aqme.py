...
from aqme.mainf import (csearch_main, geom_rules_main, qprep_main,
        ...
        if not df_qcorr_error.empty():
            for i,file_name in enumerate(df_qcorr_error['file_name']):
                ...
                if df_qcorr_error['termination'][i] != "normal" or df_qcorr_error['errortype'][i] not in [None,'isomerization']:
                    ...
                    if not args.nocom:
                        qprep(destination=dir_for_analysis, molecule=file_name,charge=df_qcorr_error['charge'][i],
                                        mult=df_qcorr_error['mult'][i], atom_types=df_qcorr_error['atom_types'][i],
        ...
        if not df_qcorr_success.empty() and args.sp != None:
            ...
            for i,file_name in enumerate(df_qcorr_success['file_name']):
                if df_qcorr_success['termination'][i] == "normal" and df_qcorr_success['errortype'][i] == None:
                    ...
                    qprep(destination=dir_for_analysis, molecule=file_name, charge=df_qcorr_success['charge'][i], 
                                    mult=df_qcorr_success['mult'][i], atom_types=df_qcorr_success['atom_types'][i],
